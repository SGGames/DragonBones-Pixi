...
export default class ReplaceSlotDisplay extends BaseDemo {
  ...
  protected _onStart(): void {
    this._factory.parseDragonBonesData(
      this._pixiResources["resource/mecha_1004d_show/mecha_1004d_show_ske.json"]
    );
    this._factory.parseTextureAtlasData(
      this._pixiResources["resource/mecha_1004d_show/mecha_1004d_show_tex.json"],
      this._pixiResources["resource/mecha_1004d_show/mecha_1004d_show_tex.png"]

    );
    this._factory.parseDragonBonesData(
      this._pixiResources["resource/weapon_1004_show/weapon_1004_show_ske.json"]
    );
    this._factory.parseTextureAtlasData(
      this._pixiResources["resource/weapon_1004_show/weapon_1004_show_tex.json"],
      this._pixiResources["resource/weapon_1004_show/weapon_1004_show_tex.png"]
    ...
    const touchHandler = (event: any) => {
      const localX = event.data.global.x - this.container.x;
      if (localX < -150.0) {
  ...
  private _replaceDisplay(type: number): void {
    if (type === -1) {
      ...
      this._armatureDisplay.armature.getSlot("weapon_hand_l")!.displayIndex =
        this._leftWeaponIndex;
    } else {
