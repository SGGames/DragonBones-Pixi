...
export class CoreElementGame extends BaseDemo {
    public static GROUND: number;
    public static G: number = 0.6;
    public static instance: CoreElementGame;
    ...
    protected _onStart(): void {
        CoreElementGame.GROUND = this.stageHeight * 0.5 - 550.0;
        ...
        factory.parseTextureAtlasData(this._pixiResources["resource/mecha_1502b/mecha_1502b_tex.json"],
            this._pixiResources["resource/mecha_1502b/mecha_1502b_tex.png"]);
        factory.parseDragonBonesData(this._pixiResources["resource/skin_1502b/skin_1502b_ske.json"]);
        factory.parseTextureAtlasData(this._pixiResources["resource/skin_1502b/skin_1502b_tex.json"],
            this._pixiResources["resource/skin_1502b/skin_1502b_tex.png"]);
        factory.parseDragonBonesData(this._pixiResources["resource/weapon_1000/weapon_1000_ske.json"]);
        factory.parseTextureAtlasData(this._pixiResources["resource/weapon_1000/weapon_1000_tex.json"],
            this._pixiResources["resource/weapon_1000/weapon_1000_tex.png"]);
        //
        this._player = new Mecha();
    }

    private _touchHandler(event: any): void {
        this._player.aim(event.data.global.x - this.container.x, event.data.global.y - this.container.y);
        console.log(event);
    ...
    private _keyHandler(event: KeyboardEvent): void {
        ...
        switch (event.keyCode) {
            case 37:
            case 65:
                CoreElementGame.instance._left = isDown;
                CoreElementGame.instance._updateMove(-1);
                break;

            case 39:
            case 68:
                CoreElementGame.instance._right = isDown;
                CoreElementGame.instance._updateMove(1);
                break;
            ...
            case 87:
                if (isDown) {
                    CoreElementGame.instance._player.jump();
                }
                break;

            case 83:
            case 40:
                CoreElementGame.instance._player.squat(isDown);
                break;

            case 81:
                if (isDown) {
                    CoreElementGame.instance._player.switchWeaponR();
                }
                break;

            case 69:
                if (isDown) {
                    CoreElementGame.instance._player.switchWeaponL();
                }
                break;

            case 32:
                if (isDown) {
                    CoreElementGame.instance._player.switchSkin();
                }
                break;
        }
    }

    private _enterFrameHandler(any): void { // Make sure game update before dragonBones update.
        if (this._player) {
            this._player.update();
        }
...
class Mecha {
    ...
    private _weaponL: Armature;
    private _weaponR: Armature;
    ...
    public constructor() {
        ...
        this._armatureDisplay.y = CoreElementGame.GROUND;
        ...
        CoreElementGame.instance.addChild(this._armatureDisplay);
    ...
    private _frameEventHandler(event: EventType): void {
        if (event.name === "fire") {
            ...
            this._helpPoint.x -= CoreElementGame.instance.container.x;
            this._helpPoint.y -= CoreElementGame.instance.container.y;
    ...
    private _updatePosition(): void {
        if (this._speedX !== 0.0) {
            ...
            if (this._armatureDisplay.x < -CoreElementGame.instance.stageWidth * 0.5) {
                this._armatureDisplay.x = -CoreElementGame.instance.stageWidth * 0.5;
            }
            else if (this._armatureDisplay.x > CoreElementGame.instance.stageWidth * 0.5) {
                this._armatureDisplay.x = CoreElementGame.instance.stageWidth * 0.5;
            }
        }

        if (this._speedY !== 0.0) {
            if (this._speedY < 5.0 && this._speedY + CoreElementGame.G >= 5.0) {
                this._armature.animation.fadeIn(
                    "jump_3", -1.0, -1, 0
                    , Mecha.NORMAL_ANIMATION_GROUP
                )!.resetToPose = false;
            ...
            this._speedY += CoreElementGame.G;
            this._armatureDisplay.y += this._speedY;

            if (this._armatureDisplay.y > CoreElementGame.GROUND) {
                this._armatureDisplay.y = CoreElementGame.GROUND;
                this._speedY = 0.0;
                this._armature.animation.fadeIn(
                    "jump_4", -1.0, -1,
...
class Bullet {
    ...
    public constructor(armatureName: string, effectArmatureName: string | null, radian: number, speed: number, position: PointType) {
        ...
        if (effectArmatureName !== null) {
            ...
            CoreElementGame.instance.addChild(this._effecDisplay);
    ...
    public update(): boolean {
        ...
        if (
            this._armatureDisplay.x < -CoreElementGame.instance.stageWidth * 0.5 - 100.0 || this._armatureDisplay.x > CoreElementGame.instance.stageWidth * 0.5 + 100.0 ||
            this._armatureDisplay.y < -CoreElementGame.instance.stageHeight * 0.5 - 100.0 || this._armatureDisplay.y > CoreElementGame.instance.stageHeight * 0.5 + 100.0
        ) {
            CoreElementGame.instance.removeChild(this._armatureDisplay);
            this._armatureDisplay.dispose();

            if (this._effecDisplay !== null) {
                CoreElementGame.instance.removeChild(this._effecDisplay);
