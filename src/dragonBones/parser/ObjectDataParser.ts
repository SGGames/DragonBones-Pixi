...
import { ActionType, ArmatureType, BoneType, ConstraintType, BlendMode, DisplayType, BoundingBoxType, TimelineType, AnimationBlendType, BinaryOffset, TweenType } from "../core/DragonBones";
...
import { GeometryData, MeshDisplayData, PathDisplayData, DisplayData, ImageDisplayData, ArmatureDisplayData, BoundingBoxDisplayData, WeightData } from "../model/DisplayData";
...
export class ObjectDataParser extends DataParser {
  ...
  protected _parseArmature(rawData: any, scale: number): ArmatureData {
    ...
    armature.frameRate = ObjectDataParser._getNumber(
      rawData,
      DataParser.FRAME_RATE,
      this._data.frameRate
    );
    armature.scale = scale;

    if (
      DataParser.TYPE in rawData &&
    ...
    if (DataParser.BONE in rawData) {
      ...
      for (const rawBone of rawBones) {
        const parentName = ObjectDataParser._getString(
          rawBone,
          DataParser.PARENT,
          ""
        );
    ...
    for (let i = 0, l = this._cacheRawMeshes.length; i < l; ++i) {
      ...
      const shareName = ObjectDataParser._getString(
        rawData,
        DataParser.SHARE,
        ""
      );
      ...
      let skinName = ObjectDataParser._getString(
        rawData,
        DataParser.SKIN,
        DataParser.DEFAULT_NAME
      );
      ...
      const shareMesh = armature.getMesh(
        skinName,
        "",
        shareName
      ) as MeshDisplayData | null; // TODO slot;
      if (shareMesh === null) {
        continue; // Error.
      }
  ...
  protected _parseIKConstraint(rawData: any): ConstraintData | null {
    const bone = this._armature.getBone(
      ObjectDataParser._getString(rawData, DataParser.BONE, "")
    );
    if (bone === null) {
      return null;
    }

    const target = this._armature.getBone(
      ObjectDataParser._getString(rawData, DataParser.TARGET, "")
    );
    if (target === null) {
      return null;
    }
  ...
  protected _parsePathConstraint(rawData: any): ConstraintData | null {
    const target = this._armature.getSlot(
      ObjectDataParser._getString(rawData, DataParser.TARGET, "")
    );
    if (target === null) {
      return null;
    ...
    constraint.positionMode = DataParser._getPositionMode(
      ObjectDataParser._getString(rawData, DataParser.POSITION_MODE, "")
    );
    constraint.spacingMode = DataParser._getSpacingMode(
      ObjectDataParser._getString(rawData, DataParser.SPACING_MODE, "")
    );
    constraint.rotateMode = DataParser._getRotateMode(
      ObjectDataParser._getString(rawData, DataParser.ROTATE_MODE, "")
    );
  ...
  protected _parseSlot(rawData: any, zOrder: number): SlotData {
    ...
    slot.parent = this._armature.getBone(
      ObjectDataParser._getString(rawData, DataParser.PARENT, "")
    ) as any; //

    if (
      DataParser.BLEND_MODE in rawData &&
  ...
  protected _parseSkin(rawData: any): SkinData {
    ...
    if (DataParser.SLOT in rawData) {
      ...
      for (const rawSlot of rawSlots) {
        const slotName = ObjectDataParser._getString(
          rawSlot,
          DataParser.NAME,
          ""
        );
  ...
  protected _parseDisplay(rawData: any): DisplayData | null {
    ...
    switch (type) {
      ...
      case DisplayType.BoundingBox: {
        ...
        if (boundingBox !== null) {
          const boundingBoxDisplay = (display = BaseObject.borrowObject(
  ...
  protected _parseBoundingBox(rawData: any): BoundingBoxData | null {
    ...
    if (
      DataParser.SUB_TYPE in rawData &&
  ...
  protected _parseAnimation(rawData: any): AnimationData {
    ...
    if (DataParser.FFD in rawData) {
      ...
      for (const rawTimeline of rawTimelines) {
        ...
        const displayName = ObjectDataParser._getString(
          rawTimeline,
          DataParser.NAME,
          ""
        );
        ...
        this._mesh = this._armature.getMesh(
          skinName,
          slotName,
          displayName
        ) as any;
    ...
    if (DataParser.IK in rawData) {
      ...
      for (const rawTimeline of rawTimelines) {
        const constraintName = ObjectDataParser._getString(
          rawTimeline,
          DataParser.NAME,
          ""
        );
    ...
    if (DataParser.TIMELINE in rawData) {
      ...
      for (const rawTimeline of rawTimelines) {
        ...
        const timelineName = ObjectDataParser._getString(
          rawTimeline,
          DataParser.NAME,
          ""
        );
  ...
  protected _parseTimeline(
    ...
    if (rawData !== null) {
      this._timelineArray[timelineOffset + BinaryOffset.TimelineScale] =
        Math.round(
          ObjectDataParser._getNumber(rawData, DataParser.SCALE, 1.0) * 100
        );
      this._timelineArray[timelineOffset + BinaryOffset.TimelineOffset] =
        Math.round(
          ObjectDataParser._getNumber(rawData, DataParser.OFFSET, 0.0) * 100
        );
    } else {
    ...
    this._timelineArray[timelineOffset + BinaryOffset.TimelineKeyFrameCount] =
      keyFrameCount;
    this._timelineArray[timelineOffset + BinaryOffset.TimelineFrameValueCount] =
      frameValueCount;

    switch (this._frameValueType) {
      case FrameValueType.Step:
        this._timelineArray[
          timelineOffset + BinaryOffset.TimelineFrameValueOffset
        ] = 0;
        break;
    ...
    if (keyFrameCount === 1) {
      ...
      this._timelineArray[
        timelineOffset + BinaryOffset.TimelineFrameOffset + 0
      ] =
      ...
      for (
        ...
        if (frameStart + frameCount <= i && iK < keyFrameCount) {
          ...
          this._timelineArray[
            timelineOffset + BinaryOffset.TimelineFrameOffset + iK
          ] =
  ...
  protected _parseBoneTimeline(rawData: any): void {
    const bone = this._armature.getBone(
      ObjectDataParser._getString(rawData, DataParser.NAME, "")
    );
    if (bone === null) {
      return;
    }
  ...
  protected _parseSlotTimeline(rawData: any): void {
    const slot = this._armature.getSlot(
      ObjectDataParser._getString(rawData, DataParser.NAME, "")
    );
    if (slot === null) {
      return;
  ...
  protected _parseTweenFrame(
    ...
    if (frameCount > 0) {
      if (DataParser.CURVE in rawData) {
        ...
        const isOmited = this._samplingEasingCurve(
          rawData[DataParser.CURVE],
          this._helpArray
        );
  ...
  protected _parseActionFrame(
    ...
    this._frameArray[frameOffset + BinaryOffset.FramePosition] = frameStart;
    this._frameArray[frameOffset + BinaryOffset.FramePosition + 1] =
      actionCount; // Action count.

    for (let i = 0; i < actionCount; ++i) {
      // Action offsets.
      this._frameArray[frameOffset + BinaryOffset.FramePosition + 2 + i] =
        frame.actions[i];
    }
  ...
  protected _parseBoneRotateFrame(
    ...
    // Modify rotation.
    let rotation =
      ObjectDataParser._getNumber(rawData, DataParser.ROTATE, 0.0) *
      Transform.DEG_RAD;
  ...
  protected _parseSlotDeformFrame(
    ...
    const vertexCount =
      this._intArray[
        this._mesh.geometry.offset + BinaryOffset.GeometryVertexCount
      ];
    const meshName =
    ...
    if (frameStart === 0) {
      ...
      this._frameIntArray[frameIntOffset + BinaryOffset.DeformVertexOffset] =
      ...
      this._timelineArray[
        this._timeline.offset + BinaryOffset.TimelineFrameValueCount
  ...
  protected _parseIKConstraintFrame(
    ...
    this._frameIntArray[frameIntOffset++] = Math.round(
      ObjectDataParser._getNumber(rawData, DataParser.WEIGHT, 1.0) * 100.0
    );
      ...
      for (const rawAction of rawData) {
        ...
        if (DataParser.GOTO_AND_PLAY in rawAction) {
          ...
          action.name = ObjectDataParser._getString(
            rawAction,
            DataParser.GOTO_AND_PLAY,
            ""
          );
        } else {
          if (
            DataParser.TYPE in rawAction &&
        ...
        if (DataParser.BONE in rawAction) {
          const boneName = ObjectDataParser._getString(
            rawAction,
            DataParser.BONE,
            ""
          );
  ...
  protected _parseDeformFrame(
    ...
    const rawVertices =
      DataParser.VERTICES in rawData
        ? (rawData[DataParser.VERTICES] as Array<number>)
        : DataParser.VALUE in rawData
          ? (rawData[DataParser.VALUE] as Array<number>)
          : null;
    const offset = ObjectDataParser._getNumber(rawData, DataParser.OFFSET, 0); // uint
    const vertexCount =
      this._intArray[this._geometry.offset + BinaryOffset.GeometryVertexCount];
  ...
  protected _parseTransform(
    ...
    if (DataParser.ROTATE in rawData || DataParser.SKEW in rawData) {
      ...
      transform.rotation = Transform.normalizeRadian(
        ObjectDataParser._getNumber(rawData, DataParser.SKEW_Y, 0.0) *
          Transform.DEG_RAD
      );
      transform.skew =
        Transform.normalizeRadian(
          ObjectDataParser._getNumber(rawData, DataParser.SKEW_X, 0.0) *
            Transform.DEG_RAD
        ) - transform.rotation;
    }
  ...
  protected _parseGeometry(rawData: any, geometry: GeometryData): void {
    ...
    this._intArray[geometryOffset + BinaryOffset.GeometryVertexCount] =
      vertexCount;
    this._intArray[geometryOffset + BinaryOffset.GeometryFloatOffset] =
      verticesOffset;
    ...
    if (DataParser.TRIANGLES in rawData) {
      ...
      for (let i = 0, l = triangleCount * 3; i < l; ++i) {
        this._intArray[
          geometryOffset + BinaryOffset.GeometryVertexIndices + i
        ] = rawTriangles[i];
      }
    }
    // Fill triangle count.
    this._intArray[geometryOffset + BinaryOffset.GeometryTriangleCount] =
      triangleCount;
    ...
    if (DataParser.WEIGHTS in rawData) {
      ...
      this._intArray[weightOffset + BinaryOffset.WeigthFloatOffset] =
        floatOffset;
      ...
      if (DataParser.BONE_POSE in rawData) {
        ...
        for (
          let i = 0,
            iW = 0,
            iB =
              weightOffset + BinaryOffset.WeigthBoneIndices + weightBoneCount,
            iV = floatOffset;
          i < vertexCount;
          ++i
        ) {
  ...
  protected _modifyArray(): void {
    ...
    const colorArray = new Uint16Array(
      binary,
      l1 + l2 + l3 + l4 + l5 + l6,
  ...
  public parseDragonBonesData(
    ...
    const version = ObjectDataParser._getString(
      rawData,
      DataParser.VERSION,
      ""
    );
    const compatibleVersion = ObjectDataParser._getString(
      rawData,
      ...
      if (DataParser.ARMATURE in rawData) {
        ...
        if (DataParser.STAGE in rawData) {
          data.stage = data.getArmature(
            ObjectDataParser._getString(rawData, DataParser.STAGE, "")
          );
  ...
  public parseTextureAtlasData(
    ...
    if (DataParser.SUB_TEXTURE in rawData) {
      ...
      for (let i = 0, l = rawTextures.length; i < l; ++i) {
        ...
        const frameWidth = ObjectDataParser._getNumber(
          rawTexture,
          DataParser.FRAME_WIDTH,
          -1.0
        );
        const frameHeight = ObjectDataParser._getNumber(
          rawTexture,
          DataParser.FRAME_HEIGHT,
          -1.0
        );
