...
export class BinaryDataParser extends ObjectDataParser {
  ...
  private _parseBinaryTimeline(
    ...
    const keyFrameCount =
      this._timelineArrayBuffer[
        timeline.offset + BinaryOffset.TimelineKeyFrameCount
      ];
    if (keyFrameCount === 1) {
          ...
          frameStart =
            this._frameArrayBuffer[
              this._animation.frameOffset +
                this._timelineArrayBuffer[
                  timeline.offset + BinaryOffset.TimelineFrameOffset + iK
                ]
            ];
  ...
  protected _parseAnimation(rawData: any): AnimationData {
    ...
    if (DataParser.BONE in rawData) {
      ...
      for (let k in rawTimeliness) {
        ...
        for (let i = 0, l = rawTimelines.length; i < l; i += 2) {
          ...
          const timeline = this._parseBinaryTimeline(
            timelineType,
            timelineOffset
          );
    ...
    if (DataParser.TIMELINE in rawData) {
      ...
      for (const rawTimeline of rawTimelines) {
        const timelineOffset = ObjectDataParser._getNumber(
          rawTimeline,
          DataParser.OFFSET,
          0
        );
  ...
  protected _parseGeometry(rawData: any, geometry: GeometryData): void {
    ...
    const weightOffset =
      this._intArrayBuffer[geometry.offset + BinaryOffset.GeometryWeightOffset];
    if (weightOffset >= 0) {
      const weight = BaseObject.borrowObject(WeightData);
      const vertexCount =
        this._intArrayBuffer[
          geometry.offset + BinaryOffset.GeometryVertexCount
        ];
      const boneCount =
        this._intArrayBuffer[weightOffset + BinaryOffset.WeigthBoneCount];
      weight.offset = weightOffset;

      for (let i = 0; i < boneCount; ++i) {
        const boneIndex =
          this._intArrayBuffer[
            weightOffset + BinaryOffset.WeigthBoneIndices + i
          ];
        weight.addBone(this._rawBones[boneIndex]);
      }

      let boneIndicesOffset =
        weightOffset + BinaryOffset.WeigthBoneIndices + boneCount;
      let weightCount = 0;
  ...
  protected _parseArray(rawData: any): void {
    ...
    const intArray = new Uint16Array(
      this._binary,
      this._binaryOffset + offsets[0],
      l1 / Uint16Array.BYTES_PER_ELEMENT
    );
