  ...
  public init(
    ...
    if (this._timelineData !== null) {
      ...
      this._timeScale =
        100.0 /
        this._timelineArray[
          this._timelineData.offset + BinaryOffset.TimelineScale
        ];
      this._timeOffset =
        this._timelineArray[
          this._timelineData.offset + BinaryOffset.TimelineOffset
        ] * 0.01;
    }
  }
  ...
  public update(passedTime: number): void {
    if (this._setCurrentTime(passedTime)) {
      if (this._frameCount > 1) {
        ...
        const frameIndex =
          this._frameIndices[
            (this._timelineData as TimelineData).frameIndicesOffset +
              timelineFrameIndex
          ];
...
export abstract class TweenTimelineState extends TimelineState {
  ...
  private static _getEasingCurveValue(
    progress: number,
    samples: Array<number> | Int16Array,
    count: number,
    offset: number
  ): number {
    if (progress <= 0.0) {
  ...
  protected _onArriveAtFrame(): void {
    if (
      ...
      this._tweenType =
        this._frameArray[this._frameOffset + BinaryOffset.FrameTweenType];
      ...
      if (this._isTween) {
        if (this._tweenType === TweenType.Curve) {
          this._curveCount =
            this._frameArray[
              this._frameOffset +
                BinaryOffset.FrameTweenEasingOrCurveSampleCount
            ];
        } else if (
          this._tweenType !== TweenType.None &&
          this._tweenType !== TweenType.Line
        ) {
          this._tweenEasing =
            this._frameArray[
              this._frameOffset +
