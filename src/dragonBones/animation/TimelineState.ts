...
import { Slot, DisplayFrame } from "../armature/Slot";
  ...
  private _onCrossFrame(frameIndex: number): void {
    ...
    if (this._animationState.actionEnabled) {
      const frameOffset =
        ...
        this._timelineArray[
          (this._timelineData as TimelineData).offset +
            BinaryOffset.TimelineFrameOffset +
            frameIndex
        ];
  ...
  public update(passedTime: number): void {
    ...
    if (this._setCurrentTime(passedTime)) {
      ...
      if (this._frameCount > 1) {
        ...
        const timelineFrameIndex = Math.floor(
        ...
        if (this._timelineData !== null) {
          this._frameOffset =
            this._animationData.frameOffset +
            this._timelineArray[
              this._timelineData.offset + BinaryOffset.TimelineFrameOffset
            ];
          // Arrive at frame.
          const framePosition =
  ...
  public init(
    ...
    if (this._timelineData !== null) {
      ...
      const frameIntOffset =
        ...
        this._timelineArray[
          this._timelineData.offset + BinaryOffset.TimelineFrameValueCount
        ];
      this._valueOffset = this._animationData.frameFloatOffset;
      this._valueCount =
        frameIntArray[frameIntOffset + BinaryOffset.DeformValueCount];
      this._deformCount =
        frameIntArray[frameIntOffset + BinaryOffset.DeformCount];
      this._deformOffset =
        frameIntArray[frameIntOffset + BinaryOffset.DeformValueOffset];
      this._sameValueOffset =
        frameIntArray[frameIntOffset + BinaryOffset.DeformFloatOffset] +
      ...
      this._deformCount = (
        (this.target as BlendState).target as Surface
      )._deformVertices.length;
    }
  }
