...
export class DisplayFrame extends BaseObject {
  ...
  public getBoundingBox(): BoundingBoxData | null {
    if (
      this.displayData !== null &&
      this.displayData.type === DisplayType.BoundingBox
    ) {
    ...
    if (
      this.rawDisplayData !== null &&
      this.rawDisplayData.type === DisplayType.BoundingBox
    ) {
      ...
      if (this._geometryData !== prevGeometryData) {
        ...
        if (this._geometryData !== null) {
          ...
          if (this._geometryData.weight !== null) {
            for (
              let i = 0, l = this._geometryData.weight.bones.length;
              i < l;
              ++i
            ) {
  ...
  protected _updateGlobalTransformMatrix(isCache: boolean): void {
    const parentMatrix =
      this._parent._boneData.type === BoneType.Bone
        ...
        : (this._parent as Surface)._getGlobalTransformMatrix(
            this.global.x,
            this.global.y
          );
  ...
  public replaceRawDisplayData(
    ...
    if (displayFrame.rawDisplayData !== displayData) {
      ...
      if (displayFrame.rawDisplayData === null) {
        ...
        if (defaultSkin !== null) {
          const defaultRawDisplayDatas = defaultSkin.getDisplays(
            this._slotData.name
          );
          if (
